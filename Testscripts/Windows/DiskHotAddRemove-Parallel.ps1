...
function Main {
    ...
    try {
        ...
        $lsvmbusCount = Run-LinuxCmd -username $user -password $password -ip $AllVMData.PublicIP -port $AllVMData.SSHPort -command "lsvmbus -v | grep -i 'Synthetic SCSI Controller' -A 2 | tail -2 | grep -i 'target' -c" -runAsSudo
        ...
        if ($lsvmbusCount = $cpuCount / 4) {
            Write-LogInfo "Multichannel is used over multiple CPU"
        } else {
            throw "Multichannel usage failed"
        }
